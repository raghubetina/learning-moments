...
interface ClaudeMetricsSummary {
  calls: number;
...
function sinceMs(value = "24h"): number {
...
function countBy(events: EventRecord[], field: string): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const event of events) {
    const value = typeof event[field] === "string" ? event[field] as string : "unknown";
    counts[value] = (counts[value] ?? 0) + 1;
  }
  return counts;
}

function summarizeClaude(events: EventRecord[]): ClaudeMetricsSummary {
  const summary: ClaudeMetricsSummary = {
    calls: events.length,
    wall_duration_ms: 0,
    duration_ms: 0,
    duration_api_ms: 0,
...
export async function metricsCommand(options: MetricsOptions): Promise<void> {
  ...
  console.log(`  median time: ${ms(summary.hooks.median_duration_ms)}`);
  console.log(`  p95 time: ${ms(summary.hooks.p95_duration_ms)}`);
  console.log(`  outcomes: ${JSON.stringify(summary.hooks.by_outcome)}`);
  console.log("");
  console.log("Classifier:");
  console.log(`  calls: ${summary.classifier.calls}`);
  console.log(`  completed: ${summary.classifier.completed}`);
  console.log(`  declined: ${summary.classifier.declined}`);
  console.log(`  failed open: ${summary.classifier.failed_open}`);
  console.log(`  duplicate candidates skipped: ${summary.classifier.duplicate_candidates}`);
  ...
  console.log("");
  console.log("Grader:");
  console.log(`  calls: ${summary.grader.calls}`);
  console.log(`  failed open: ${summary.grader.failed_open}`);
  ...
  console.log("");
  console.log("User:");
  console.log(`  moments injected: ${summary.user.moments_injected}`);
  console.log(`  answered: ${summary.user.answered}`);
  console.log(`  skipped: ${summary.user.skipped}`);
  console.log(`  grades: ${summary.user.grades}`);
}

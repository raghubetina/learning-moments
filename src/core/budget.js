function since(events, windowMs, now = new Date()) {
  const cutoff = now.getTime() - windowMs;
  return events.filter((event) => new Date(event.timestamp).getTime() >= cutoff);
}

export function immediatePromptBudgetAvailable(events, config, now = new Date()) {
...
export function classifierBudgetAvailable(events, config, now = new Date()) {
